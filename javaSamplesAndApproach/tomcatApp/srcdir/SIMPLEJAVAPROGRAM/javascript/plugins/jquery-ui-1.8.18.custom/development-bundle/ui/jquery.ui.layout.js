...
,	panes: { // default options for 'all panes' - will be overridden by 'per-pane settings'
	...
	,	slideDelay_open:		500			// applies only for mouseenter event - 0 = instant open
	,	slideDelay_close:		500			// applies only for mouseleave event (300ms is the minimum!)
	...
	*/
	...
	,	onopen_end:				function() {
								if(gMaskConfirm == true)
								{
									
									if(displayedChartType != "Radar Chart" && displayedChartType != "Grid" && displayedChartType != "Pie Chart" && displayedChartType != "Gauge Chart" )									
                                                            			setProperty();
                                                            			
                                                            		if(displayedChartType == "Radar Chart")
                                                            		{
                                                            			dijit.byId("accordion_wrapper_radar").resize();//resizing accordion
                                                            			displayRadarChart();
                                                            		}
                                                            			
                                                            		if(displayedChartType == "Grid")
                                                            		{
                                                            			dijit.byId("accordion_wrapper_grid").resize();//resizing accordion
                                                            		}
                                                            			
                                                            		if(displayedChartType == "Pie Chart")
                                                            		{
                                                            			dijit.byId("accordion_wrapper_pie").resize();//resizing accordion
                                                            		}
                                                            			
                                                            		if(displayedChartType == "Gauge Chart")
                                                            		{
                                                            			dijit.byId("accordion_wrapper_gauge").resize();//resizing accordion
                                                            			createGuageChart();
                                                            		...
                                                            		{
                                                            			dijit.byId("accordion_wrapper_grid").resize();//resizing accordion
                                                            			createChart(XMLvalues,'Grid');
                                                            		}
                                                            		if(displayedChartType == "BulletChart")
                                                            		{
                                                            			dijit.byId("accordion_wrapper_bullet").resize();//resizing accordion
                                                            			createChart(XMLvalues,"BulletChart");//createChart.js 
                                                            		}                                                            		
	...
	,	onclose_end:			function() {
								if(gMaskConfirm == true)
								{
									if(displayedChartType != "Radar Chart" && displayedChartType != "Grid" && displayedChartType != "Pie Chart" && displayedChartType != "Gauge Chart" )
                                                        ...
                                                        } 	// CALLBACK when pane ENDS being Closed
